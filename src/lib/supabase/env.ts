...
export const APP_URL = process.env.NEXT_PUBLIC_APP_URL ?? "https://lauralounge.com";

// Stable base used to build the printed QR codes. Set this to your OWN domain
// (lauralounge.com) so printed QR codes never need reprinting when the
// underlying deployment changes — just re-point the domain. Falls back to
// APP_URL / the current origin when unset.
export const QR_BASE_URL = process.env.NEXT_PUBLIC_QR_BASE_URL?.trim() ?? "https://lauralounge.com";
const SINGLE_RESTAURANT_FALLBACK_ID = "8f7c1bf8-feb5-4f34-88fb-781f2fd89d55";

// Application mono-restaurant: L'Aura Lounge.
